...
<body>
    <div class="container">
        ...
        <div class="main-content">
            ...
            <?php
            // Menangani penghapusan foto
            if (isset($_POST['remove']) && isset($_POST['file_path'])) {
                $filePath = $_POST['file_path'];

                // Menghapus file dari direktori unggahan
                if (file_exists($filePath)) {
                    unlink($filePath); // Menghapus file
                }

                // Menghapus catatan file dari database
                $stmt = $conn->prepare("DELETE FROM media WHERE file_path = ?");
                $stmt->bind_param("s", $filePath);
                $stmt->execute();
                $stmt->close();

                echo "<p>Foto berhasil dihapus!</p>";
            }
            ?>
                ...
                <div class="gallery">
                    <?php foreach ($photos as $photo): ?>
                        <div class="photo-box">
                            ...
                            <form action="media.php" method="post" style="text-align: center; margin-top:5px;">
                                <input type="hidden" name="file_path" value="<?php echo $photo; ?>">
